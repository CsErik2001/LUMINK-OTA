...
class Screen():
    ...
    def __repr__(self):
        print("screen width: %d" % self.width)
        print("screen height: %d" % self.height)
        print("screen width bytes: %d" % self.width_bytes)
        print("screen height bytes: %d" % self.height_bytes)
...
class Paint():
    ...
    def draw_rectangle(self, x_start, y_start, x_end, y_end, fill=False):
        if fill:
    ...
    def show_char(self, char, x_start, y_start, font=asc2_0806, font_size=(6, 8), multiplier=1):
        tmp = 0x00
        if char == '°':
            char_idx = len(font) - 1  # Utolsó karakter a font-ban (° jel)
        else:
    ...
    def show_string(self, string, x_start, y_start, font=asc2_0806, font_size=(6, 8), multiplier=1, invert=False):
        text_color = Color.WHITE if (
                self.bg_color == Color.BLACK and invert) else Color.BLACK if invert else self.bg_color

        original_bg_color = self.bg_color
        self.bg_color = text_color

        for idx, char in enumerate(string):
            self.show_char(char, x_start + idx * font_size[0] * multiplier, y_start, font, font_size, multiplier)

        self.bg_color = original_bg_color

    def show_bitmap(self, bitmap, x_start, y_start, multiplier=1, invert=False):
        if multiplier == 1:
            for r, row in enumerate(bitmap):
                for c, col in enumerate(row):
                    if (col == 1 and not invert) or (col == 0 and invert):
                        self.draw_point(x_start + c, y_start + r)
        else:
            for r in range(len(bitmap) * multiplier):
                for c in range(len(bitmap[0]) * multiplier):
                    if (bitmap[r // multiplier][c // multiplier] == 1 and not invert) or (
                            bitmap[r // multiplier][c // multiplier] == 0 and invert):
                        self.draw_point(x_start + c, y_start + r)
...
class SSD1680():
    def __init__(self, spi, dc, busy, cs, res, rotate=Rotate.ROTATE_270):
        super().__init__()
        self.spi = spi
        self.dc = dc
        self.busy = busy
        ...
        self.res = res
        self.screen = Screen()
        self.paint = Paint(self.screen, rotate=rotate, bg_color=Color.WHITE)
